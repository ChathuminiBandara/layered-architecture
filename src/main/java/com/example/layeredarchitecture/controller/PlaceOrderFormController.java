...
public class PlaceOrderFormController {
    ...
    orderDao placeOrderDao = new orderImpl();
...
//                            "There is no such customer associated with the id " + id
                        ...
                        CustomerDTO customerDTO = customerDAO.setCustomerName(newValue);
...
//                        throw new NotFoundException("There is no such item associated with the id " + code);
                    ...
                    ArrayList<ItemDTO> item = placeOrderDao.orderDetail(newItemCode);

                    for (int i = 0; i < item.size(); i++) {
                        txtDescription.setText(item.get(i).getDescription());
                        txtUnitPrice.setText(item.get(i).getUnitPrice().setScale(2).toString());
...
//                    txtQtyOnHand.setText(tblOrderDetails.getItems().stream().filter(detail-> detail.getCode().equals(item.getCode())).<Integer>map(detail-> item.getQtyOnHand() - detail.getQty()).findFirst().orElse(item.getQtyOnHand()) + "");
                        ...
                        txtQtyOnHand.setText((optOrderDetail.isPresent() ? item.get(i).getQtyOnHand() - optOrderDetail.get().getQty() : item.get(i).getQtyOnHand()) + "");
                    }
    ...
    private boolean existItem(String code) throws SQLException, ClassNotFoundException {
          ...
          boolean itemExit = itemDao.exitItem(code);
          return itemExit;
    }

    boolean existCustomer(String id) throws SQLException, ClassNotFoundException {

        boolean exitCustomer = customerDAO.existCustomer(id);
        return exitCustomer;
    }
    ...
    public String generateNewOrderId() {
        try {
            ...
            ResultSet rst = placeOrderDao.generateNewId();
    ...
    private void loadAllCustomerIds() {
        try {
            ArrayList<CustomerDTO> customerIdList = customerDAO.getAllCustomer();


            for (CustomerDTO customerID : customerIdList) {
                cmbCustomerId.getItems().add(customerID.getId());
            }
    ...
    private void loadAllItemCodes() {
        try {
            /*Get all items*/
            ArrayList<String> itemCodeList = itemDao.loadAllItemCodes();
    ...
    public boolean saveOrder() {
        ...
        boolean saveOrder = placeOrderDao.saveOrder(orderId,LocalDate.now(),cmbCustomerId.getValue(),tblOrderDetails.getItems().stream().map(tm -> new OrderDetailDTO(tm.getCode(), tm.getQty(), tm.getUnitPrice())).collect(Collectors.toList()));
        return saveOrder;
    }
}
